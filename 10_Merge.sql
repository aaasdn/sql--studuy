...
SELECT * FROM depts;
MERGE INTO depts tb -- (머지를 할 타겟 테이블)
    USING -- 병합시킬 데이터
        (SELECT * FROM departments) b -- 병합하고자 하는 데이터를 서브쿼리로 표현.
    ON -- 병합시킬 데이터의 연결 조건
        (tb.department_id = b.department_id)
WHEN MATCHED THEN -- 조건이 일치하는 경우에는 타겟 테이블에 이렇게 실행하라.
    UPDATE SET 
        tb.department_name = b.department_name,
        tb.manager_id = b.manager_id,
        tb.location_id = b.location_id
...
-- <5-4>
MERGE INTO jobs_it tb
    USING
    (SELECT * FROM jobs
    WHERE min_salary > 5000) a 
    ON(tb.job_id = a.job_id)
WHEN MATCHED THEN
    UPDATE SET
    tb.min_salary = a.min_salary,
    tb.max_salary = a.max_salary
WHEN NOT MATCHED THEN
    INSERT VALUES

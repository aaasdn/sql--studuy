...
-- 하나라도 만족하면 됩니다. (조건에 만족을 해야함 (그래서 부등호를 씀))
SELECT * FROM employees
WHERE salary > ANY /* == SOME */ (SELECT salary FROM employees
                 WHERE first_name = 'David');
...
-- EXISTS: 서브쿼리가 하나 이상의 행을 반환하면 참으로 간주. (단순 존재 유무 파악)
...
수정, 삭제 등이 빈번한 경우 (sql 가독성면에서 조인이 좀 더 뛰어납니다.)
*/   
